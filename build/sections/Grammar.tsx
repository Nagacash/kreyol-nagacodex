...
const Grammar: React.FC = () => {
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold tracking-tight text-primary font-serif">Grammar Essentials</h1>
      ...
      >
        {GRAMMAR_DATA.map((rule) => (
          <motion.div key={rule.title} variants={itemVariants}>
            <Card>
              <h2 className="text-2xl font-semibold mb-2 text-primary font-serif">{rule.title}</h2>
              <p className="text-muted-foreground mb-4">{rule.explanation}</p>
              <div className="space-y-3">
                {rule.examples.map((example, index) => (
                  <div key={index} className="bg-secondary/50 p-3 rounded-md group">
                    <div className="flex justify-between items-center">
                        <div>
                            <p className="font-mono text-base text-primary">{example.creole}</p>
                            <p className="text-sm text-muted-foreground">
                              <span className="font-semibold">EN:</span> {example.english} | <span className="font-semibold">FR:</span> {example.french}
                            </p>
                        </div>
                         <button
