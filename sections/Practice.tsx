...
const Practice: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col">
      <h1 className="text-4xl font-bold tracking-tight text-foreground mb-2 font-serif">AI Practice Partner</h1>
      ...
      <Card className="flex-1 flex flex-col p-0">
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          <AnimatePresence>
            {messages.map((msg, index) => (
              ...
              >
                ...
                <div
                  className={`max-w-xl p-3 rounded-lg whitespace-pre-wrap text-base ${
                    msg.sender === 'user'
          ...
          {error && <div className="text-destructive text-base text-center">{error}</div>}
          <div ref={messagesEndRef} />
        </div>
        <div className="border-t border-border p-4 bg-card-dark">
          <div className="relative">
            <input
              ...
              className="w-full px-4 py-2 pr-12 border border-input rounded-full bg-secondary focus:ring-2 focus:ring-ring focus:outline-none text-base placeholder:text-muted-foreground"
              disabled={isLoading}
            />
            <button
              onClick={handleSend}
