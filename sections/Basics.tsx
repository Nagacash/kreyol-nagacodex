...
const PhraseCard: React.FC<{ phrase: Phrase }> = ({ phrase }) => (
  <motion.div variants={itemVariants}>
    <Card className="p-0 hover:shadow-lg transition-shadow group">
      <div className="flex justify-between items-start p-6">
        <div>
          <p className="text-xl font-semibold text-foreground">{phrase.creole}</p>
          <p className="text-base text-muted-foreground">French: {phrase.french}</p>
          <p className="text-base text-muted-foreground">English: {phrase.english}</p>
        </div>
        <button
